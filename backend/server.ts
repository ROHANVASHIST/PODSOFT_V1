...
async function startServer() {
    ...
    try {
      ...
      if (isTimeout) {
        res.status(504).json({ 
          ...
          details: `The request to ${url} timed out (30s). Private network IPs are NOT accessible from cloud environments unless running locally.`,
          code: "TIMEOUT"
        });
      } else if (isNetworkError) {
        res.status(502).json({ 
          error: "Source Unreachable", 
          details: `Could not reach ${url}. Ensure the DroidCam app is open and the IP/Port is correct.`,
          code: "UNREACHABLE"
        });
      } else {
        res.status(500).json({ 
  ...
  app.post("/api/generate-layout-specs", async (req, res) => {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Generate a JSON layout for an OBS-style scene based on this prompt: "${prompt}". Coordinates: x/y from 0 to 1920 and 0 to 1080.`
      });
  ...
  app.post("/api/upload-chunk", express.raw({ type: 'video/webm', limit: '50mb' }), (req, res) => {
    ...
    console.log(`Received chunk ${index} for recording ${recordingId}. Size: ${req.body ? req.body.length : 0} bytes`);
    res.json({ success: true, url: `/api/recordings/${recordingId}/${index}.webm` });
  });

  server.listen(PORT, "0.0.0.0", () => {
    console.log(`PodSoft API Server running on port ${PORT}`);
  });
}
