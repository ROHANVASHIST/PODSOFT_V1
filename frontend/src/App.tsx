...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadStudio = async () => {
      ...
      try {
        const { data: studios, error } = await supabase.from('studios').select('*').eq('ownerId', user.uid).limit(1);
        if (error && error.code !== 'PGRST116') throw error;

        if (!studios || studios.length === 0) {
          const { data: newStudioData, error: insertErr } = await supabase.from('studios').insert({
            name: `${user.displayName || 'Default'}'s Studio`,
            ownerId: user.uid,
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
          }).select().single();
          if (insertErr || !newStudioData) throw insertErr || new Error("Failed to create studio");

          setStudioId(newStudioData.id);
          
          await supabase.from('scenes').insert({
            studioId: newStudioData.id,
            name: 'Scene 1',
            order: 0,
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
        ...
        handleSupabaseError(error, OperationType.LIST, 'studios');
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchScenes = () => {
      supabase.from('scenes').select('*').eq('studioId', studioId).order('order', { ascending: true })
        .then(({ data, error }) => {
          if (error) handleSupabaseError(error, OperationType.LIST, 'scenes');
          else if (data) {
            setScenes(data.map(s => ({ ...s, sources: [] })));
    ...
    const channel = supabase.channel('scenes_channel')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'scenes', filter: `studioId=eq.${studioId}` }, () => {
        supabase.from('scenes').select('*').eq('studioId', studioId).order('order', { ascending: true })
          .then(({ data }) => {
            if (data) setScenes(prev => data.map(s => ({ ...s, sources: prev.find(p => p.id === s.id)?.sources || [] })));
          });
      }).subscribe();
  ...
  useEffect(() => {
    ...
    const fetchItems = () => {
      supabase.from('sceneItems').select('*').in('sceneId', sceneIds)
        .then(({ data, error }) => {
          if (error) handleSupabaseError(error, OperationType.LIST, 'sceneItems');
          else if (data) {
    ...
    const channel = supabase.channel('items_channel')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'sceneItems' }, () => {
        fetchItems();
      }).subscribe();

    return () => { supabase.removeChannel(channel); };
  }, [scenes.length, streams]);

  // 4. Sync Recording List
  useEffect(() => {
    if (!studioId) return;
    const fetchRecs = () => {
      supabase.from('recordings').select('*').eq('studioId', studioId).order('startTime', { ascending: false }).limit(20)
        .then(({ data }) => { if (data) setRecordings(data); });
    };
    fetchRecs();

    const channel = supabase.channel('recs_channel')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'recordings', filter: `studioId=eq.${studioId}` }, () => {
        fetchRecs();
      }).subscribe();
  ...
  const smartBuildScene = async () => {
    ...
    try {
      ...
      // 1. Create the scene
      const { data: newSceneData, error: sceneErr } = await supabase.from('scenes').insert({
        studioId,
      ...
      if (sceneErr || !newSceneData) throw sceneErr || new Error("Failed to create scene");

      // 2. Add the items
      for (const src of data.sources) {
        await supabase.from('sceneItems').insert({
          sceneId: newSceneData.id,
          name: src.name,
          type: src.type,
          x: src.x,
          y: src.y,
      ...
      setActiveSceneId(newSceneData.id);
  ...
  const addSource = async (type: Source['type'], deviceId?: string, extraData?: any) => {
    try {
      ...
      const { data: newDoc, error: insertErr } = await supabase.from('sceneItems').insert(newSourceData).select().single();
      if (insertErr || !newDoc) throw insertErr || new Error("Insert failed");
      
      if (stream) {
        setStreams(prev => ({ ...prev, [newDoc.id]: stream! }));
      }
  ...
  const toggleVisibility = async (sourceId: string) => {
    ...
    if (!source) return;
    try {
      await supabase.from('sceneItems').update({
        visible: !source.visible,
        updatedAt: new Date().toISOString()
      }).eq('id', sourceId);
    } catch (error) {
      handleSupabaseError(error, OperationType.UPDATE, `sceneItems/${sourceId}`);
    }
  };
  ...
  const handleReorderSource = async (sourceId: string, direction: 'up' | 'down') => {
    ...
    try {
      await supabase.from('sceneItems').update({ order: newIndex }).eq('id', sourceToMove.id);
      await supabase.from('sceneItems').update({ order: currentIndex }).eq('id', targetSource.id);
    } catch (error) {
      handleSupabaseError(error, OperationType.UPDATE, 'sceneItems');
    }
  };

  const deleteSource = async (sourceId: string) => {
    try {
      await supabase.from('sceneItems').delete().eq('id', sourceId);
      setStreams(prev => {
        const next = { ...prev };
      ...
      handleSupabaseError(error, OperationType.DELETE, `sceneItems/${sourceId}`);
    }
  };
  ...
  const startRecording = async () => {
    ...
    let recId = '';
    try {
      const { data: recDoc, error } = await supabase.from('recordings').insert({
        studioId,
      ...
      if (e.data.size > 0) {
        ...
        if (recId) {
          ...
          fetch(`/api/upload-chunk?recordingId=${recId}&index=${index}`, {
            ...
            .then(data => {
              supabase.from('recording_chunks').insert({
                recordingId: recId,
                chunkIndex: index,
                url: data.url,
                size: blob.size,
    ...
    recorder.onstop = () => {
      if (recId) {
        supabase.from('recordings').update({
          status: 'completed',
    ...
    switch (transitionType) {
      ...
      case 'Swipe':
        return {
          ...
          animate: { x: 0 },
          exit: { x: '-100%' },
          transition: { duration, ease: "easeInOut" } as any
        };
      case 'Slide':
        return {
        ...
        {isMobile && currentView === 'studio' && (
          <div className="flex-1 flex flex-col">
            ...
            <div className="h-16 bg-obs-surface border-t border-obs-border flex items-center justify-around px-4">
               <NavBtn icon={<Layout size={20}/>} label="Studio" active={(currentView as string) === 'studio'} onClick={() => setCurrentView('studio')} />
               <NavBtn icon={<FolderOpen size={20}/>} label="Files" active={(currentView as string) === 'file'} onClick={() => setCurrentView('file')} />
               <NavBtn icon={<Database size={20}/>} label="Drive" active={(currentView as string) === 'drive'} onClick={() => setCurrentView('drive')} />
               <NavBtn icon={<Activity size={20}/>} label="Edit" active={(currentView as string) === 'edit'} onClick={() => setCurrentView('edit')} />
               <NavBtn icon={<UserIcon size={20}/>} label="Profile" active={(currentView as string) === 'profile'} onClick={() => setCurrentView('profile')} />
            </div>
          </div>
        )}
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex-1 max-w-[45%] h-full flex flex-col gap-2">
              ...
              <div className="flex gap-1 overflow-x-auto pb-1">
                {Array.from(new Map<string, Source>(scenes.flatMap(s => s.sources).map(s => [s.id, s])).values()).map(s => (
                  <button
                    key={s.id}
              ...
              {(() => {
                const allSources: Source[] = Array.from(new Map<string, Source>(scenes.flatMap(s => s.sources).map(s => [s.id, s])).values());
                            ...
                            <SourceVideo 
                              ...
                              onVolumeChange={async (vol, muted) => {
                                try {
                                  ...
                                  handleSupabaseError(error, OperationType.UPDATE, `sceneItems/${source.id}`);
                                }
                              }}
                            />
              ...
              <div className="pt-1 flex gap-2 border-t border-obs-border text-obs-text-dim bg-obs-surface/50 -mx-1 px-2">
                <Plus size={14} className="hover:text-white cursor-pointer" onClick={async () => {
                  try {
                    ...
                    handleSupabaseError(error, OperationType.CREATE, 'scenes');
                  }
                }} />
                <Sparkles 
                  size={14} 
                ...
                <Trash2 size={14} className="hover:text-white cursor-pointer" onClick={async () => {
                  if (scenes.length > 1) {
                    try {
                      await supabase.from('scenes').delete().eq('id', activeSceneId);
                    } catch (error) {
                      handleSupabaseError(error, OperationType.DELETE, `scenes/${activeSceneId}`);
                    }
                  }
                }} />
                   ...
                   <FilterSlider 
                     ...
                     onChange={async (v) => {
                       try {
                         ...
                         handleSupabaseError(error, OperationType.UPDATE, `sceneItems/${selectedSourceId}`);
                       }
                     }}
                   />
                   <FilterSlider 
